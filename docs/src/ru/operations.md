...
Предусловие: `inv setup-replica` уже был выполнен против VM 2.
...
### Проверка свежести: `inv backup-status`

Самый неприятный сценарий — таймер тихо перестал отрабатывать,
а всё остальное выглядит здоровым. `inv backup-status` — внешний
зонд, который живёт не на VM 2:

```bash
inv backup-status                     # одна строка, exit 0/1
inv backup-status --json-output       # машиночитаемый вердикт
inv backup-status --max-age-hours 3   # понижаем порог во время инцидента
```
...
backup-status`, и при ненулевом коде возврата передаёт захваченный
...
- Установлен `rclone` (`apt install rclone` на Ubuntu, `brew
  ...
  ничего не нужно.
- На операторской машине настроен удалённый `yandex:` в rclone,
  указывающий на тот же аккаунт Яндекс.Диска, что и у
  `inv setup-replica-backup`. Если `inv setup-replica-backup` ни
  разу не запускался с этой машины, настроить его разово:
  `rclone config create yandex webdav url=https://webdav.yandex.ru vendor=other user=<login>`
  (запросит app-пароль через `rclone obscure`).
