...
локального restore), добавляет позже `inv backup-cloud-setup` — см.
...
- На операторской машине настроен удалённый `yandex:` в rclone,
  ...
  `inv backup-cloud-setup`. Если `inv backup-cloud-setup` ни
...
- Три слоя избыточности, каждый под свой сценарий отказа:
  ...
  Яндекс.Диск (`inv backup-cloud-setup`) — «ой, я потерял обе VM /
