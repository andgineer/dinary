...
- Установлен `rclone` (`apt install rclone` на Ubuntu, `brew
  ...
  `inv setup-server`, так что в стрессе disaster recovery ставить руками
  ничего не нужно.
- На операторской машине настроен удалённый `yandex:` в rclone,
  указывающий на тот же аккаунт Яндекс.Диска, что и у
  `inv setup-replica`. Если `inv setup-replica` ни
...
- Установлены `sqlite3` и `zstd` (на VM 1 — через `inv setup-server`, на
...
5. По желанию `inv verify-db` для проверки целостности, затем
   `inv migrate --remote` для подтверждения, что восстановленная БД на
