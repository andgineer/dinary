...
Cloudflare Tunnel создаёт зашифрованное соединение от машины с dinary до сети Cloudflare. Это даёт HTTPS и кастомный домен без открытия портов в файрволе. Cloudflare Access добавляет авторизацию по email (бесплатно до 50 пользователей).

Все команды в шагах 1-5 выполняются **на машине, где работает dinary** (Oracle Cloud VM, ваш Mac/PC и т.д.).
...
- dinary запущен на `http://localhost:8000` на целевой машине.
