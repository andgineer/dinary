...
Чату нужен ключ `OPENAI_API_KEY` в `.deploy/.env` — чат работает на одной платной модели OpenAI, а не на бесплатных провайдерах конвейера чеков. Без ключа диаграммы работают в штатном режиме — только в области чата появляется предупреждение.
