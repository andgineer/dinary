...
Чат требует хотя бы один ключ LLM-провайдера в `.deploy/.env` — те же ключи, что использует конвейер чеков. Без него диаграммы работают в штатном режиме — только в области чата появляется предупреждение.
