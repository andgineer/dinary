...
3. Сохраните скачанный файл как `credentials.json` в корне проекта (для локальной разработки) или загрузите на хостинг.
...
Для Railway (нет поддержки файлов-секретов) можно закодировать JSON-ключ в base64 и указать `DINARY_GOOGLE_CREDENTIALS_BASE64` — см. [инструкцию по развёртыванию на Railway](deploy-railway.md#4-добавление-ключа-сервисного-аккаунта).

!!! note
    При использовании `docker compose` с `.env` указывайте `GOOGLE_SHEETS_SPREADSHEET_ID` (без префикса `DINARY_`) — `docker-compose.yml` добавляет префикс автоматически. На Render/Railway устанавливайте полное имя `DINARY_GOOGLE_SHEETS_SPREADSHEET_ID` напрямую в панели хостинга.
