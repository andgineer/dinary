...
| `DINARY_SHEET_LOGGING_SPREADSHEET` | опциональный ID или URL таблицы для append-only логгинга runtime-расходов (см. раздел 7) |

Конфигурация bootstrap import вынесена из `.env` в отдельный файл — см. следующий раздел.

### Источники для bootstrap import

Bootstrap import (`inv import-config`, `inv import-catalog`, `inv import-budget`, `inv import-budget-all`, `inv import-income`, `inv import-income-all`, `inv verify-bootstrap-import`, `inv verify-bootstrap-import-all`, `inv verify-income-equivalence-all`) читает список исходных таблиц из **опционального** файла `.deploy/import_sources.json` в корне репозитория. Файл `gitignore`-нут — в репозиторий утекают только плейсхолдеры из `.deploy.example/import_sources.json`.

Если вы не собираетесь запускать bootstrap import (например, начинаете с пустой DuckDB и вводите расходы только через PWA), файл можно не создавать — runtime без него работает. В таком случае сид таксономии делает `inv bootstrap-catalog`.

Чтобы включить bootstrap import, скопируйте шаблон и отредактируйте:
...
Формат — JSON-массив, по одному объекту на год:
...
- `worksheet_name` — имя листа для import расходов (по умолчанию пусто → первый видимый лист).
- `layout_key` — парсер формата листа (`default`, `rub_6col`, `rub_2016`, `rub_2014`, `rub_2012`, `rub_fallback`); по умолчанию подбирается по году.
- `income_worksheet_name` — опциональное имя листа для import доходов.
- `income_layout_key` — опциональный парсер для import доходов (`balance_rub`, `balance_rub_rsd`, `balance_rsd`, `income_rsd`).
- `notes` — необязательный свободный комментарий для операторов.

`.deploy/import_sources.json` — единственный источник правды о том, какие годы существуют: `inv import-*` и `inv verify-*` берут список лет именно из этого файла (раньше он лежал в таблице DuckDB `import_sources` — таблица убрана, миграция на файл прошла в 2026-04). Пока файл не существует, любая команда `inv import-*` упадёт с указанием на `imports/` в корне репозитория, где лежит полная документация схемы и workflows.
