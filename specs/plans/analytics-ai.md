...
10. **In-session LLM tools in `dashboard.py`** (Gemini chat tool definitions):
    ...
    - `save_current_view(name)` → persists draft via `settings.save_view()`, then
      pushes the saved config to `PUT /api/analytics/config` as key `active_view`
...
14. **Suggested questions feature** — add `update_suggestions(questions: list[str])`
