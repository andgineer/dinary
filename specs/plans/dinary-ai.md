...
- `test_db_snapshot_cleans_up_temp_file_on_backup_failure` — patch `sqlite3.Connection.backup` to raise; assert the route surfaces an error response and the temp `.db` file is not left behind
...
Define `class RefreshError(Exception)` directly in `refresh.py`, where it is both raised and caught — a dedicated `exceptions.py` module would be indirection this codebase doesn't use for a single exception class (cf. `ClassificationExhaustedError` in `background/classification/task.py`, `AllProvidersBusyError` in `adapters/llm_chat.py`: both defined in the module that raises them; `api/controllers/catalog_writer_errors.py` is a dedicated errors module, but it groups a *family* of four related exceptions, not one).

Redefine **both** `REPLICA_PATH` and `ANALYTICS_DB_PATH` in `src/dinary_analytics/paths.py` to be platform-specific, resolved via a shared `_app_data_dir() -> Path` helper keyed on `sys.platform`. Neither stays server-relative (`_DATA_DIR / "ledger-replica.db"` / `_DATA_DIR / "analytics.db"`) — and this has to apply to both, not just the replica: `analytics.db` (view configs, dashboard config, LLM chat history) is read and written by several consumers — the notebook `inv analytics` launches, the background service `setup-dinary-ai` installs, and the `backup-analytics`/`restore-analytics`/`*-yadisk` tasks (`tasks/backups/analytics_backup.py`, via `dinary_analytics.backup`) — and all of them must agree on *one* location for state they touch. The sharpest version of this is the notebook vs. the service: pinning `ANALYTICS_DB_PATH` to `_DATA_DIR = Path(settings.data_path).parent` ties it to whatever directory each process happens to be running from; the notebook's CWD is wherever the user runs `inv analytics` from, while the service's `WorkingDirectory` is frozen at `inv install-dinary-ai` time (Step 3) — if those ever drift apart (a re-clone, a move, a second checkout), the notebook and the service silently start reading and writing two different `analytics.db` files with no error. A platform-keyed `_app_data_dir()` (home directory + `sys.platform`, not CWD) converges all of these consumers on the same path regardless of where each one is invoked from. `_app_data_dir()` returns:
...
`REPLICA_PATH = _app_data_dir() / "dinary-ai.db"`; `ANALYTICS_DB_PATH = _app_data_dir() / "analytics.db"`. With both constants moved off it, `_DATA_DIR` (and its `from dinary.config import settings` import) has no remaining use in `paths.py` — delete both; `pyrefly` would otherwise flag the now-dead import. (`dinary-ai` so far runs only on the repo owner's laptop — no other machine has installed it yet, so there is nothing to migrate: both `data/analytics.db` and `data/ledger-replica.db` can simply be deleted, not carried over. `_open_env` (`settings.py:15`, `lmdb.open(..., create=True)`) creates a fresh `analytics.db` LMDB env at the new `ANALYTICS_DB_PATH` the moment anything first writes to it, so the repo owner starts from an empty `analytics.db` on the new path — same as anyone installing `dinary-ai` for the first time — losing only the handful of locally-saved view configs and chat history. `data/ledger-replica.db` needs nothing special either: it's reproducible, already excluded from backup by spec, and the daemon writes a fresh one to the new location on its first refresh.)
...
- Streams the response body in chunks to a sibling temp path, `tmp_path = db_path.with_suffix(".tmp")`.
- Validates the download before trusting it: reads the first 16 bytes of `tmp_path` and compares them against SQLite's file-format magic header (`b"SQLite format 3\x00"`); a mismatch raises `RefreshError("downloaded snapshot is not a valid SQLite database")` (after `tmp_path.unlink(missing_ok=True)`) instead of proceeding to the replace below. Without this check, a 200 response whose body isn't actually a database — a captive-portal redirect, a proxy's error page, a stream the server completed with status 200 despite truncating it — would pass the status check, then silently clobber a working replica with garbage via the atomic replace, breaking every query until the next successful refresh.
- Only once that check passes: `os.replace(tmp_path, db_path)` — atomic on both POSIX and Windows, so a reader (DuckDB via `open_ledger()`) opening `db_path` mid-download always sees either the previous complete file or the new complete one, never a partial write.
- Returns `db_path`.
...
Tests in `tests/analytics/test_refresh.py`. The module-level state (`_db_path`, `_last_refresh`, `_last_refresh_error`, `_daemon_thread`, `_wake_event`) is a singleton shared with real background threads, so a fixture must reset all of it (via `monkeypatch.setattr` on each name, plus `_wake_event.clear()`) before and after every test in this file — otherwise a thread spawned by one `test_start_refresh_daemon_*`/`test_refresh_loop_*` test keeps mutating shared state while a later test runs against it. Resetting state alone isn't enough to neutralise an already-running thread, though — it would keep calling whatever `refresh_replica` the *next* test has patched in, racing its assertions. So every `test_refresh_loop_*`/`test_start_refresh_daemon_*` test that spins up a real `_refresh_loop` patches `refresh_replica` with a `side_effect` sequence ending in a sentinel exception that is **not** `RefreshError` — e.g. a locally defined `class _StopLoop(Exception)` — so that once the test has observed what it needs, the next call propagates uncaught (the loop only catches `RefreshError`) and the thread exits; the test then `thread.join(timeout=...)` before asserting, proving the thread is dead and cannot pollute whatever runs next.
- `test_app_data_dir_darwin` — with `monkeypatch.setattr(sys, "platform", "darwin")`, `_app_data_dir()` resolves under `~/Library/Application Support/dinary`
- `test_app_data_dir_win32` — with `sys.platform` patched to `"win32"` and `LOCALAPPDATA` set via `monkeypatch.setenv`, resolves under that directory's `dinary` subfolder
- `test_app_data_dir_win32_raises_without_localappdata` — `sys.platform` patched to `"win32"`, `LOCALAPPDATA` deleted via `monkeypatch.delenv`, raises `RuntimeError("LOCALAPPDATA not set; cannot determine DB path on Windows")`
- `test_app_data_dir_linux_fallback` — with `sys.platform` patched to `"linux"`, resolves to `~/.local/share/dinary` (the XDG path that keeps the Linux CI leg's import green, per the note above)
...
- `test_refresh_replica_raises_on_invalid_snapshot` — a 200 response whose body doesn't start with the SQLite magic header raises `RefreshError("downloaded snapshot is not a valid SQLite database")`, leaves the previous `db_path` untouched, and removes the `.tmp` file
...
launchd plist: `~/Library/LaunchAgents/dev.dinary.ai.plist`, `Label: dev.dinary.ai`, `KeepAlive: true`, `RunAtLoad: true`, `WorkingDirectory: <repo_root>`. The `Label` must be exactly `dev.dinary.ai` — it is the service identifier `setup-dinary-ai`'s `launchctl kickstart -k gui/$(id -u)/dev.dinary.ai` and `launchctl print gui/$(id -u)/dev.dinary.ai` target (above), and a mismatch (e.g. a typo, or reusing the filename including `.plist`) would make those commands silently fail to find the agent. The working directory is required, not optional: launchd starts agents with `cwd=/`, and `uv run` resolves `pyproject.toml` and the project's venv relative to cwd — without `WorkingDirectory` the daemon fails to start. `<repo_root>` is computed the same way as elsewhere in `tasks/`: `Path(__file__).resolve().parents[1]` from `tasks/dinary_ai.py`.
...
- `test_install_writes_plist` — plist file written to correct path with expected keys, including `Label` (must equal `dev.dinary.ai`, matching the `launchctl` target used elsewhere in this step) and `WorkingDirectory`, on macOS
...
A new top-most cell in `dashboard.py`, rendered *above* the snapshot-status banner described below — on a freshly set-up machine `get_app_url()` (Step 1) returns `None`, and the banner's `/health` check would otherwise greet the user with "Replica not ready" before they've had any chance to tell `dinary-ai` where the server is. Add `from dinary_analytics.refresh import get_app_url, set_app_url, trigger_refresh_now` to the notebook's top-cell imports.

This step (gate plus banner, below) also needs three stdlib symbols the notebook's top-level import cell doesn't currently carry — add them there alongside the `dinary_analytics` imports above, never locally inside a cell: `import urllib.request` and `import urllib.error` (the banner's `/health` and `/refresh/now` calls need both — the notebook has no `urllib` import today), and extend the existing `from datetime import date, timedelta` to `from datetime import date, datetime, timedelta, timezone` (`_format_last_refresh` below needs `datetime.now(tz=timezone.utc)` and to parse the ISO 8601 `last_refresh` string — neither `datetime` nor `timezone` is in scope today; `json` already is, see the banner section).
...
2. If `True` — the steady state: every run after the first successful save, every run on a machine that already has `LOCAL_CONFIG_PATH` from a previous session, and every run after the user later re-opens and re-saves the address via the banner's "Change server address" control (below) — renders `mo.md("")` and lets the rest of the notebook proceed untouched. This is "потом все это скрывается": once configured, the gate stays hidden — the banner's control, not a hand-edited config file, is the only way back to this form.
3. If `False`, builds the setup form: an `mo.callout` walking the user through finding the server's address — the same territory `docs/src/en/pwa-install.md` already covers for installing the PWA on a phone, because it's the same address. Install Tailscale on this machine if it isn't already there (the same download-page / app-store links `pwa-install.md` gives for the phone case) and log in with the same account used on the server; then open the Tailscale app (or https://login.tailscale.com/admin/machines), find the server machine in the list, and its MagicDNS name *is* the address to enter — e.g. `https://dinary-host.tailxxxx.ts.net`, the exact value the user already pasted into their phone's browser to reach the PWA. Below the instructions: `mo.ui.text(label="Server address", placeholder="https://dinary-host.tailxxxx.ts.net", value=get_app_url() or "")` and `mo.ui.button(label="Save", on_click=...)`. The `value=` pre-fill matters the second time this form appears — via "Change server address" below, `get_app_url()` returns the very address the user is here to correct, so they see and edit what's already saved instead of retyping it from scratch; on a first-ever run `get_app_url()` is `None`, so `or ""` leaves the field blank as before. The handler strips the input; a non-empty value calls `set_app_url(url)`, then `trigger_refresh_now()` — without this, the daemon would sit out whatever backoff delay it has already accumulated while unconfigured (Step 1: `RefreshError` retries start at `REFRESH_RETRY_BASE_SECONDS = 30` and double up to `REFRESH_INTERVAL_SECONDS = 86400`, so by the time a user has worked through the Tailscale instructions above, the next scheduled retry could be many minutes away) — then `set_address_configured(True)`. An empty value re-renders the same form with an inline `mo.callout(mo.md("Please enter the server address."), kind="warn")` instead — mirroring how `prompt_yandex_credentials` (`tasks/backups/backups_yandex.py`) refuses an empty answer, just via a re-rendered form rather than `sys.exit` (a notebook cell has no process to exit).
4. `mo.stop(not address_configured(), <the rendering from step 2 or 3>)` — halts the gate cell's own descendants until the address is saved. Saving flips `address_configured` to `True`; the cell re-runs, step 2 fires, and the gate hides again — until the banner's "Change server address" control (below) flips `address_configured` back to `False`, re-running this cell straight back to step 3 with the form pre-filled for correction.
...
`mo.stop` only short-circuits a cell's own reactive descendants (cells that read its return values) — it has no effect on cells that merely happen to sit below it visually. So the status cell must take an explicit dependency on the gate's state, or it would run its `/health` check independently and stack a "Replica not ready" callout underneath the address-setup form on a freshly set-up machine — exactly the double-message the gate exists to prevent. The status cell therefore also reads `address_configured` *and* `set_address_configured` (the same `mo.state` pair the gate cell uses — the getter so this cell re-runs the moment the gate flips it and stays hidden while the form shows, the setter so its own "Change server address" control, below, can flip the gate back open) and opens with `mo.stop(not address_configured(), mo.md(""))`: while the gate is showing its form, the status cell renders nothing and performs no `/health` check at all; the instant `set_address_configured(True)` fires, this cell re-runs, the `mo.stop` no longer fires, and the health check below proceeds.

Having cleared that gate, the status cell also reads `refresh_ticker` and `refresh_requested` — so it re-runs every 5 seconds *and* immediately when the button below is clicked — and on each run:
...
2. If `refresh_requested()` is `True`: issues a `POST` to `http://localhost:{MCP_PORT}/refresh/now` via `urllib.request.urlopen(urllib.request.Request(url, method="POST"))` — plain `urlopen(url)` defaults to `GET` and would 405 against a route registered only for `POST` (fire-and-forget — `trigger_refresh_now()` wakes the daemon and the route returns immediately, Step 2), then immediately calls `set_refresh_requested(False)` so the trigger fires once per click, not on every following tick.
...
6. Otherwise renders the banner and lets the notebook continue — e.g. `mo.hstack([mo.md(f"🔄 {_format_last_refresh(last_refresh)}"), mo.ui.button(label="Refresh now", on_click=lambda _: set_refresh_requested(True)), mo.ui.button(label="⚙ Change server address", on_click=lambda _: set_address_configured(False))])`. The third button is the only way back into the address-setup gate once it has hidden itself: clicking it flips `address_configured` to `False`, which simultaneously makes this cell's own `mo.stop(not address_configured(), mo.md(""))` (above) fire — hiding the banner and skipping its `/health` poll, exactly as on a freshly set-up machine — and makes the gate cell re-run straight to step 3, its form now pre-filled via `value=get_app_url() or ""` (gate, point 3) with the address being corrected. No config file to find or hand-edit; saving the corrected address flips `address_configured` back to `True` and both cells return to their steady states. `_format_last_refresh` is a small helper, defined as a local function inside the status cell — the same way `period_date_from` (`dashboard.py:443`) is defined locally inside its cell as the notebook's only comparable small formatting helper today — that picks one of two phrasings based on how long ago the ISO 8601 `last_refresh` was relative to `datetime.now(tz=timezone.utc)`:
...
Because the user can now force a sync on demand, the background poll only has to bound staleness for people who never click the button — hence `REFRESH_INTERVAL_SECONDS = 86400` (Step 1): routine freshness comes from the button, and the once-a-day poll is just a safety net for whoever never presses it.
