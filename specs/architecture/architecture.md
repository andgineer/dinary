...
The authoritative SQL lives in [src/dinary/db/migrations/0001_initial_schema.sql](../../src/dinary/db/migrations/0001_initial_schema.sql); the block below is a current snapshot. Migrations are applied by a thin SQLite backend for `yoyo-migrations` ([src/dinary/db/db_migrations.py](../../src/dinary/db/db_migrations.py)) that pins `PRAGMA journal_mode=WAL` / `PRAGMA foreign_keys=ON` and wraps every migration in `BEGIN IMMEDIATE` so `busy_timeout` can take effect at the start of the txn instead of surfacing `SQLITE_BUSY` at COMMIT.
...
-- in `settings.app_currency`, RSD, since that is what the PWA sends).
-- When `currency_original == accounting_currency`, identity FX keeps
-- `amount` and `amount_original` equal.
...
-- Stored in `settings.accounting_currency` (default EUR).
CREATE TABLE income (
...
- The initial tag set is the hardcoded `PHASE1_TAGS` list in `seed_config.py`; `import_mapping_tags` rows reference those same tags by id (the seed ordering upserts `tags` before building the mapping rebuild), so they never contribute extra names. After the first `inv import-catalog`, the set grows/shrinks via the Admin API (`POST /api/catalog/tags`, `PATCH /api/catalog/tags/{id}`). The PWA discovers tags via `GET /api/catalog` and supports a multi-select on the expense form.
...
For each write, the server preserves `(amount_original, currency_original)` verbatim for audit and stores the NBS-anchored conversion to the accounting currency in `amount`. When `currency_original == accounting_currency` the conversion is identity and the two values are bit-equal. When they differ (e.g. the typical PWA case `RSD → EUR`), the server converts at the NBS middle rate for the expense date. Historical bootstrap import performs the same conversion, per row, using the NBS rate on the 1st of the expense month, via an RSD-anchored two-step conversion so every row of `expenses.amount` is in the accounting currency regardless of which decade it was imported from. An additional EUR projection is retained for the legacy housing heuristic and the 2D→3D diagnostic report, but is never stored in `expenses`; when the accounting currency is EUR it is bit-identical to `amount`.
...
2. **Admin API (`catalog_writer`)** — every `POST /api/catalog/<kind>` and `PATCH /api/catalog/<kind>/<id>` mutation runs through `src/dinary/api/controllers/catalog_writer.py`, which computes a sha256 of the canonical catalog state before and after the mutation and bumps `catalog_version` only if the hash changed. A no-op rewrite (re-adding an already-active name, renaming a row to its current name) does **not** bump.

**Scope — what is and isn't part of `catalog_version`:** the hash canonicalises the shape of `category_groups`, `categories`, `events` (including `auto_tags`), and `tags`. It deliberately does **not** cover `sheet_mapping`/`sheet_mapping_tags` or `import_mapping`/`import_mapping_tags`. Runtime 3D→2D routing (the `map` tab, reloaded by `sheets/sheet_mapping.py`) and the derived per-year `import_mapping` rebuild (owned by `seed_classification_catalog`) are internal server state that the PWA never reads directly, so there is nothing for a client to invalidate when they change. Conversely, a pure `/api/catalog/reload-map` or a no-op `inv import-config` that only refreshes mappings leaves `catalog_version` untouched, and the PWA's ETag-validated `GET /api/catalog` keeps serving `304 Not Modified`. If the mapping tables ever surface on an API the PWA reads, the hash definition in `catalog_writer.hash_catalog_state` is the single place to extend.
...
`/api/catalog/*` or `/api/llm/*` on any credential. The
...
publicly. Until then, **any** request that reaches `/api/catalog/*` or `/api/llm/*`
...
* `add_*` on a name already in use: if the existing row is inactive, it is reactivated with its other columns (sort_order / date_from / date_to / auto_attach_enabled / sheet_name / sheet_group / auto_tags) preserved — to change those, the caller uses `edit_*`. On reactivation, `add_category` does apply the caller's `group_id` because it is part of the "where am I putting this" intent of the add. Response includes `status ∈ {"created", "reactivated", "noop"}`. The reactivate branch still validates the caller's body (`date_from <= date_to` for events, `auto_tags` names resolve to existing `tags.name` rows — active or inactive) even though the values are discarded: an invalid body surfaces as 422, not as a silently-ignored 200. This keeps `add_event` symmetric with `edit_event` and prevents "PWA sent garbage, server dropped it under a successful reactivate" failure modes.
* `edit_*` is atomic within a single transaction: validations (not-found, name conflict, inactive-group, in-use guard, post-patch `date_from <= date_to`) all run **before** any UPDATE, so a failed validation never leaves the row half-edited. `edit_tag(name=...)` additionally cascades the rename into every `events.auto_tags` JSON array that references the old name (inside the same transaction), so the auto-attach pipeline keeps working without the operator having to touch each event by hand; without the cascade, `resolve_event_auto_tag_ids` would log "unknown tag name" WARNs and silently drop the renamed tag from every new expense created under that event.
* **`is_active` semantics on tags / categories / events is "hide from the ручной пикер in the PWA"** — nothing more. Event `auto_tags` keep resolving to inactive tags (so "отпуск"-style tags can be hidden from the manual picker while still auto-attaching via the vacation event), the `map` tab validates its name references against the *full* catalog (inactive rows included), and `_require_known_tag_names` accepts inactive names in `auto_tags`. The only `is_active` gate on the write path is `POST /api/expenses`' `_validate_tags` / `_validate_category` / `_validate_event`: a user-supplied (not auto-injected) tag / category / event id must be active unless the request is an idempotent replay whose stored body matches. Hard-deleted (physically absent) names remain the only thing that blocks a reload / auto-attach.
* `DELETE /api/catalog/<kind>/<id>` applies hard-vs-soft based on **all** FK-bearing tables, not just the ledger: a category / event / tag is hard-deleted only when no `expenses`/`expense_tags` row **and** no `sheet_mapping`/`sheet_mapping_tags`/`import_mapping`/`import_mapping_tags` row references it; otherwise it is soft-deleted (`is_active = FALSE`) to keep the FK graph consistent. `usage_count` in the response still reflects the ledger count only (that is what the PWA phrases as "used by N expenses"); mapping references are an implementation detail of the hard-delete safety check.
* Groups are special: `DELETE /api/catalog/groups/<id>` refuses (409) while any category — active or inactive — still points at it, because leaving an active category under an inactive group would be a semantic lie. Operators soft- or hard-delete the child categories first.
...
Runtime 3D -> 2D projection lives in the `sheet_mapping` / `sheet_mapping_tags` tables. These are not derived from historical sheet data but from a hand-curated `map` worksheet tab in the logging spreadsheet, reloaded lazily via `sheet_mapping.ensure_fresh()` (Drive API `modifiedTime` check) and eagerly via `POST /api/catalog/reload-map`.

`GET /api/catalog` returns **every** catalog row (active and inactive) and stamps each with an `is_active` flag so the PWA can filter client-side and expose a per-picker "Показать неактивные" toggle plus an "Активировать" action (`PATCH /api/catalog/<kind>/<id>` with `{"is_active": true}`). This keeps retired vocabulary reachable on demand without polluting the default UI.
...
`GET /api/llm/status` exposes per-provider usage (requests today / daily limit, rate-limited-until). `llm_providers` rows are managed via Admin API from the PWA; env vars `DINARY_LLM_*` seed the initial row on first boot only (same pattern as `accounting_currency`).
...
- On a transient error (network timeout, 5xx, `ConnectionError`, non-4xx `gspread.APIError`) the breaker arms with an exponential backoff starting at 60s and capped at 30min (`_BACKOFF_INITIAL_SEC` / `_BACKOFF_MAX_SEC` in [src/dinary/background/sheet_logging/sheet_logging.py](../../src/dinary/background/sheet_logging/sheet_logging.py)).
...
The `sheet_mapping` / `sheet_mapping_tags` tables are derived state; the source of truth is the `map` worksheet tab in the logging spreadsheet. `src/dinary/sheets/sheet_mapping.py` owns two refresh entry points:
...
| B | Sum-formula in RSD — extended in place by `append_expense_atomic` (`=460+373+...`). The worker derives the per-expense RSD amount from the expense row: if `currency_original == "RSD"` we append `amount_original` verbatim (bit-identical to what the operator typed in the PWA, the common case), otherwise we convert `amount` from `settings.accounting_currency` to RSD using the NBS rate for the expense date. The column is independent of the accounting currency so switching `accounting_currency` does not rewrite historical sheet values. |
| C | EUR conversion formula `=IF(H{r}="","",B{r}/H{r})`. This stays a sheet-side approximation (column B divided by the manual H rate) and is intentionally not reconciled against `expenses.amount` — small drift vs. DB totals is expected and accepted. |
...
- Single `data/dinary.db` file with the **3D classification schema** (`category`, `event`, `tags[]`) from day one. No per-year partitioning, no separate config DB. One migration stream at `src/dinary/db/migrations/0001_initial_schema.sql`.
...
- Historical data import (2012–present) imported via destructive `inv import-budget` / `inv import-budget-all` and verified via `inv import-verify-bootstrap` / `inv import-verify-bootstrap-all`. Bootstrap-imported rows populate `sheet_category` / `sheet_group` as audit provenance and carry `client_expense_id = NULL`.
