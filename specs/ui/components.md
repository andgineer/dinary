...
| Component | File | Contract |
|---|---|---|
| `ExpenseForm` | `components/ExpenseForm.vue` | The entire entry form: hero row (currency / amount / date), `CategoryQuickPicks` pills + `category-pick-btn` (replaces the old group→category dropdowns), event, tags, comment. Exposes `save()`, `reset()` via `defineExpose`. |
...
| `CategorySheet` | `components/CategorySheet.vue` | Searchable bottom sheet listing every active category, grouped by parent group, with LLM `suggestions` pinned to the top (each prefixed with a `Sparkles` glyph). Autofocuses the search input on open. The search input is `position: sticky; top: 0` inside the scroll container so it stays visible regardless of scroll position — it must never be scrolled out of view. Props: `open`, `suggestions`, `title`. Emits: `select(categoryId)`, `close`. |
