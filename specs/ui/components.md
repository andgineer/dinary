...
| Component | File | Contract |
|---|---|---|
| `App` | `App.vue` | Top-level shell — dev banner, sticky header (`Dinary` brand + `HeaderSegmented`), queue strip (amber full-width strip below header row, only when queue non-empty, tap → `QueueModal`), offline notice strip (below queue strip when offline), main view router by `tab`, `QueueModal`, global toast. Version string removed from header. Renders `OnboardingTemplate` instead of header+main while no category template is active yet, and renders nothing during the initial active-template check (see `screens.md#onboarding`). |
| `OnboardingTemplate` | `views/OnboardingTemplate.vue` | Full-screen chooser shown when no category template is active yet: a language selector above `TemplateList`. One tap applies the chosen template and language and continues into the app. |
...
| `CategorySheet` | `components/CategorySheet.vue` | Full-height searchable sheet listing the active template's visible categories, grouped by group and ordered by group display order. LLM `suggestions` pinned to the top with a `Sparkles` glyph and a blue-tinted border. Search input is `position: sticky; top: 0`, autofocuses on open, ~300ms debounced; results split into in-set rows and a "Not in your set" section for inactive/hidden matches — tapping an addable result activates (or unhides) it, then selects it. A category that activates with no resolvable group shows inline labelled "без группы / ungrouped". A Manage toggle switches the body to per-category hide/rename/move and per-group "add category", plus a "Switch category set →" row that expands into `TemplateList`. Props: `open`, `suggestions`, `title`. Emits: `select(categoryId)`, `close`. |
| `TemplateList` | `components/TemplateList.vue` | Renders each category template's localized name and tagline ("this is you if…"), marks the active one with a check, emits `apply(code)` on tap. Shared by `OnboardingTemplate` and `CategorySheet`'s "Switch category set" panel. Props: `templates`, `activeCode`, `lang`. |
