...
| Component | File | Contract |
|---|---|---|
| `App` | `App.vue` | Top-level shell — dev banner, sticky header (`Dinary` brand + `HeaderSegmented`), queue strip (amber full-width strip below header row, only when queue non-empty, tap → `QueueModal`), out-of-set nudge banner (below queue strip when triggered, "Switch category set" + dismiss), offline notice strip (below the other strips when offline), main view router by `tab`, `QueueModal`, `TemplateSwitchSheet`, global toast. Version string removed from header. Renders `OnboardingTemplate` instead of header+main while no category template is active yet, and renders nothing during the initial active-template check (see `screens.md#onboarding`). |
| `OnboardingTemplate` | `views/OnboardingTemplate.vue` | Full-screen chooser shown when no category template is active yet: a language selector above `TemplatePreviewPicker`. Applying a template applies the chosen language too and continues into the app. |
...
| Component | File | Contract |
|---|---|---|
| `ExpenseForm` | `components/ExpenseForm.vue` | The entry form. Hero row (currency pill `--expense` + amount input + cal+date), grouped `category-card` (`CategoryQuickPicks` row at top, `category-pick-btn` row below — name/placeholder + a muted cog that opens `CategorySheet` directly in Manage mode + chevron that opens it in pick mode — both inside one 12-px-radius shell with `--field` fill and an internal divider), Event section (`event-chips` flow with `--expense` selected fill + `+` / cog actions), Tags section (`TagPicker` + `+` / cog), comment single-line input. Exposes `save()`, `reset()` via `defineExpose`. |
...
| `CategorySheet` | `components/CategorySheet.vue` | Full-height searchable sheet listing the active template's visible categories, grouped by group and ordered by group display order. LLM `suggestions` pinned to the top with a `Sparkles` glyph and a blue-tinted border. Search input is `position: sticky; top: 0`, autofocuses on open, ~300ms debounced; results split into in-set rows and a "Not in your set" section for inactive/hidden matches — tapping an addable result activates (or unhides) it, then selects it. A category that activates with no resolvable group shows inline labelled "без группы / ungrouped". A persistent footer bar (every mode) holds a "Category set: {active набор}" button opening `TemplateSwitchSheet`, and a Manage toggle that switches the body to per-category hide/rename/move and per-group "add category". Props: `open`, `suggestions`, `title`, `initialManage` (opens straight into Manage mode). Emits: `select(categoryId)`, `close`. |
| `TemplatePreviewPicker` | `components/TemplatePreviewPicker.vue` | Master-detail набор preview. A chip row selects a template (localized name, active one checked); a detail panel below shows the selected template's full contents — every group with all its visible categories, localized. An apply button emits the choice, disabled while applying or when the selection is already active. Props: `templates`, `lang`, `activeCode`, `applying`. Emits: `apply(code)`. Shared by `OnboardingTemplate` and `TemplateSwitchSheet`. |
| `TemplateSwitchSheet` | `components/TemplateSwitchSheet.vue` | Single app-wide sheet (mounted once in `App`) wrapping `TemplatePreviewPicker`, opened via the catalog store's `templateSwitchOpen` flag from `CategorySheet`'s footer or the out-of-set nudge banner. Loads the template catalog on first open; applying re-themes the active набор, persists the chosen language, and shows a success toast. |
...
| `resolveUiLang` | `composables/uiLang.js` | Resolves the category-template UI language: first two letters of `navigator.language`, lowercased, falling back to `ru` if absent from the available language set. Used by `OnboardingTemplate` and `CategorySheet`'s "Switch category set" panel. |
| `recordOutOfSetActivation` | `composables/oosNudge.js` | Tracks "Not in your set" category activations as timestamps in localStorage; once 3 fall within the last 30 days, shows a one-time info toast pointing at "Manage → Switch category set" and resets the counter. |
