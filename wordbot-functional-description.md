...
7. When generation completes, the backend sends the pronunciation as a
   ...
   Anki via AnkiConnect, and appends a status line to the analysis
   message: "✅ added to Anki" / "📌 already in Anki" / "🕓 Anki is not
   running — card queued".
...
- **Source priority**: a real native-speaker recording when one exists —
  free dictionary sources (Wiktionary audio, e.g. via the free dictionary
  API). When no recording exists (phrases, rare words), fall back to a
  local free TTS engine (e.g. edge-tts) with a fixed voice.
- **Delivery**:
  - Telegram: a short voice message right after the analysis, so the user
    hears the word immediately.
  - Anki: the audio file is stored in the collection's media folder via
    AnkiConnect and attached to the card front, so it plays during review.
...
5. Should decks be switchable per message/session (e.g. separate deck for
   idioms)?
6. Preferred accent for pronunciation (British vs American) — pick one,
