...
from dinary.services.classification_rules import create_or_update_rule
...
def _correct_category_sync(
    ...
    try:
        ...
        try:
            ...
            # Collect all batch items (keyed by expense_id) for split/merge.
            # items_by_expense[old_expense_id] = [(item_id, receipt_id, total_price), ...]
...
def _split_merge_expenses(
    ...
    For each old expense containing batch items:
    - If all its items are moving: just update category (no amount change).
    - If only some items move: subtract moved amount from old expense and
      add it to an existing or new expense of the target category.
...
def _upsert_rule_in_tx(
    ...
    create_or_update_rule(con, store_id, item_name_normalized, category_id, 4, "user_correction")
