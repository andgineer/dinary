"""Tests for LLMBrokerStorage: provider loading, seeding, call logging, rate limiting."""
...
from dinary.adapters.llm_storage import LLMBrokerStorage, _label_from_base_url
...
@allure.feature("LLMBrokerStorage")
class TestLoadProviders:
    def test_returns_enabled_providers_sorted_by_priority(self, fresh_db):
        ...
        providers = asyncio.run(LLMBrokerStorage().load_providers())
    ...
    def test_seeds_from_toml_when_empty(self, fresh_db, tmp_path, monkeypatch, real_llm_seed):  # noqa: ARG002
        ...
        providers = asyncio.run(LLMBrokerStorage(providers_toml=toml).load_providers())
    ...
    def test_seeds_from_env_when_toml_absent(self, fresh_db, tmp_path, monkeypatch, real_llm_seed):  # noqa: ARG002
        ...
        providers = asyncio.run(LLMBrokerStorage(providers_toml=missing).load_providers())

        assert len(providers) == 1
        assert providers[0].label == "Groq"
        assert providers[0].api_key == "gsk_test"
    ...
    def test_example_toml_is_valid(self):
        ...
        providers = asyncio.run(LLMBrokerStorage(providers_toml=example).load_providers())
...
class TestOnCallLogged:
    def test_inserts_row_into_llmbroker_call_log(self, fresh_db):
        ...
        asyncio.run(LLMBrokerStorage().on_call_logged(event))
...
class TestOnRateLimited:
    def test_updates_rate_limited_until_on_provider(self, fresh_db):
        ...
        asyncio.run(LLMBrokerStorage().on_rate_limited(provider_id, until))
