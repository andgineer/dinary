...
class TestLoggingQueue:
    ...
    def test_claim_then_clear(self, populated_catalog):
        ...
        con = storage.get_connection()
        try:
            token = claim_logging_job(con, pk)
            assert token is not None
            assert clear_logging_job(con, pk, token) is True
    ...
    def test_clear_with_wrong_token_returns_false(self, populated_catalog):
        ...
        con = storage.get_connection()
        try:
            claim_logging_job(con, pk)
            assert clear_logging_job(con, pk, "wrongtoken") is False
        finally:
            con.close()
    ...
    def test_release_returns_to_pending(self, populated_catalog):
        ...
        try:
            token = claim_logging_job(con, pk)
            assert release_logging_claim(con, pk, token) is True
            row = con.execute(
    ...
    def test_double_claim_blocked(self, populated_catalog):
        ...
        con = storage.get_connection()
        try:
            t1 = claim_logging_job(con, pk)
            t2 = claim_logging_job(con, pk)
    ...
    def test_stale_claim_recoverable(self, populated_catalog):
        ...
        try:
            ...
            t1 = claim_logging_job(con, pk, now=now)
            assert t1 is not None
            future = now + timedelta(hours=1)
            t2 = claim_logging_job(
                con,
                pk,
                now=future,
    ...
    def test_list_filters_fresh_in_progress_but_resurfaces_stale(
        ...
        try:
            now = datetime(2026, 1, 1, 12)
            token = claim_logging_job(con, pk, now=now)
            assert token is not None
    ...
    def test_poison_marks_row(self, populated_catalog):
        ...
        con = storage.get_connection()
        try:
            poison_logging_job(con, pk, "boom")
            row = con.execute(
    ...
    def test_force_clear_wipes_row(self, populated_catalog):
        ...
        con = storage.get_connection()
        try:
            assert force_clear_logging_job(con, pk) is True
            assert count_logging_jobs(con) == 0
            # Already gone — idempotent false on re-delete.
            assert force_clear_logging_job(con, pk) is False
        finally:
            con.close()
